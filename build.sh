...
export KBUILD_BUILD_USER="RezaAdiPangestu"
...
set_naming() {
	if [ -d "$KERNEL_DIR"/KernelSU ]; then
		KERNEL_NAME="AncientKernel-surya-kernelsu-$ZIP_DATE"
		export ZIP_NAME="$KERNEL_NAME.zip"
	else
		KERNEL_NAME="AncientKernel-surya-$ZIP_DATE"
		export ZIP_NAME="$KERNEL_NAME.zip"
	fi
}
...
send_tg_msg() {
	tg_post_msg "<b>Docker OS: </b><code>$DISTRO</code>" \
	            ...
	            "<b>Device : </b><code>Poco X3 (surya)</code>" \
